...
type SharedAccordionProps = Omit<
  AccordionPrimitive.Root.Props<string>,
  "defaultValue" | "multiple" | "onValueChange" | "value"
>;

type AccordionProps = SharedAccordionProps & {
  ...
  return (
    <AccordionPrimitive.Root
      ...
      onValueChange={(nextValue, eventDetails) => {
        if (!multiple && !collapsible && nextValue.length === 0) {
          eventDetails.cancel();
          return;
        }

        onValueChange?.(multiple ? nextValue : (nextValue[0] ?? ""), eventDetails);
      }}
      {...props}
    />
  );
  ...
  return (
    <AccordionPrimitive.Header className="flex">
      <AccordionPrimitive.Trigger
        ...
        className={cn(
          "group/accordion-trigger flex flex-1 items-start justify-between gap-4 rounded-md py-4 text-left text-sm font-medium transition-all outline-none hover:underline focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 aria-disabled:pointer-events-none aria-disabled:opacity-50 **:data-[slot=accordion-trigger-icon]:ml-auto **:data-[slot=accordion-trigger-icon]:size-4 **:data-[slot=accordion-trigger-icon]:text-muted-foreground",
          className,
        )}
        {...props}
      >
